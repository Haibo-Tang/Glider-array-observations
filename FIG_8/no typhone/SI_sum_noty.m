...
for i=1:length(K1000K003_SI_sort)
    if  datenum(2017,7,28,0,0,0)>K1000K003_time_sort(i)|K1000K003_time_sort(i)>datenum(2017,8,2,0,0,0)
    if K1000K003_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
            if normalizedhorizontalcoordinate_gliderlon_K1000K003_sort(i)>((jj-1)*lon_lat_inter+aim_eddy_area(1,1))...
                ...
                eddy_numbe_SI(ii,jj)=eddy_numbe_SI(ii,jj)+1;
            end
        end
    end
    end
...
for i=1:length(K1000K004_SI_sort)
    if  datenum(2017,7,28,0,0,0)>K1000K004_time_sort(i)|K1000K004_time_sort(i)>datenum(2017,8,2,0,0,0)   
    if K1000K004_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
for i=1:length(K1000K005_SI_sort)
      if  datenum(2017,7,28,0,0,0)>K1000K005_time_sort(i)|K1000K005_time_sort(i)>datenum(2017,8,2,0,0,0)
    if K1000K005_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
for i=1:length(K1000K007_SI_sort)
    if  datenum(2017,7,28,0,0,0)>K1000K007_time_sort(i)|K1000K007_time_sort(i)>datenum(2017,8,2,0,0,0)
    if K1000K007_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
for i=1:length(K1000K008_SI_sort)
    if  datenum(2017,7,28,0,0,0)>K1000K008_time_sort(i)|K1000K008_time_sort(i)>datenum(2017,8,2,0,0,0)
    if K1000K008_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
for i=1:length(K1000J008_SI_sort)
    if  datenum(2017,7,28,0,0,0)>K1000J008_time_sort(i)|K1000J008_time_sort(i)>datenum(2017,8,2,0,0,0)
    if K1000J008_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
for i=1:length(K1000J007_SI_sort)
    if  datenum(2017,7,28,0,0,0)>K1000J007_time_sort(i)|K1000J007_time_sort(i)>datenum(2017,8,2,0,0,0)
    if K1000J007_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
for i=1:length(K1000J005_SI_sort)
    if  datenum(2017,7,28,0,0,0)>K1000J005_time_sort(i)|K1000J005_time_sort(i)>datenum(2017,8,2,0,0,0)
        if K1000J005_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
for i=1:length(K1000J003_SI_sort)
 if  datenum(2017,7,28,0,0,0)>K1000J003_time_sort(i)|K1000J003_time_sort(i)>datenum(2017,8,2,0,0,0)    
    if K1000J003_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
for i=1:length(K300K004_SI_sort)
    if  datenum(2017,7,28,0,0,0)>K300K004_time_sort(i)|K300K004_time_sort(i)>datenum(2017,8,2,0,0,0)
    if K300K004_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
for i=1:length(K300K003_SI_sort)
    if  datenum(2017,7,28,0,0,0)>K300K003_time_sort(i)|K300K003_time_sort(i)>datenum(2017,8,2,0,0,0)    
    if K300K003_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
for i=1:length(K300K001_SI_sort)
         if  datenum(2017,7,28,0,0,0)>K300K001_time_sort(i)|K300K001_time_sort(i)>datenum(2017,8,2,0,0,0)
    if K300K001_SI_sort(i)>sort_value;           
    for ii=1:M% 按照行循环
        for jj=1:N
...
[m, n]=size(eddy_numbe_SI);
eddy_inter_SI=[eddy_numbe_SI zeros(m,1)];
eddy_num_site_SI=[eddy_inter_SI ;zeros(1,n+1)];% 矩阵还原
...
hcs=pcolor(eddx,eddy,eddy_numbe_SI)
...
for  i=1:length(text_lon)
    for j=1:length(text_lat)
        if eddy_numbe_SI(j,i)>0
        text(text_lon(i)-0.025,text_lat(j),num2str(eddy_numbe_SI(j,i)),'color','k','fontweight','bold','linewidth',10)
        end
    end
end
