...
eddy_numbe_GISI=zeros(M,N);
eddy_numbe_GISICI=zeros(M,N);
    ...
    for ii=1:M% 按照行循环
        for jj=1:N
            if normalizedhorizontalcoordinate_gliderlon_K1000K003_sort(i)>((jj-1)*lon_lat_inter+aim_eddy_area(1,1))...
                ...
                eddy_numbe_GISI(ii,jj)=eddy_numbe_GISI(ii,jj)+1;
            end
        end
    end
    end
...
[m, n]=size(eddy_numbe_GISI);
eddy_inter_GISI=[eddy_numbe_GISI zeros(m,1)];
eddy_num_site_GISI=[eddy_inter_GISI ;zeros(1,n+1)];% 矩阵还原
% end

figure
set(0,'defaultfigurecolor','w');
hcs=pcolor(eddx,eddy,eddy_numbe_GISI)
...
for  i=1:length(text_lon)
    for j=1:length(text_lat)
        if eddy_numbe_GISI(j,i)>0
        text(text_lon(i)-0.025,text_lat(j),num2str(eddy_numbe_GISI(j,i)),'color','k','fontweight','bold','linewidth',10)
        end
    end
end
